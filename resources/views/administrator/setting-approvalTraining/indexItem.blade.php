                            ...
                            <tbody>
                                @foreach($dataItem as $no => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('administrator.setting-approvalTraining.editItem', ['id' => $item->id]) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-search-plus"></i> edit</button></a>
                                            
                                            <form action="{{ route('administrator.setting-approvalTraining.destroyItem',['id' => $item->id])}}" onsubmit="return confirm('Delete this data?')" method="post" style="float: left;">
                                                {{ csrf_field() }}                                              
