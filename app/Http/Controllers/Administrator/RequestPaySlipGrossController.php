...
class RequestPaySlipGrossController extends Controller
{
    ...
    public function submit(Request $request, $id)
    {
        ...
        foreach($bulanItem as $k => $i)
        {
            ...
            $items   = \DB::select(\DB::raw("SELECT payrollgross_history.*, month(created_at) as bulan FROM payrollgross_history WHERE MONTH(created_at)=". $i->bulan ." and user_id=". $data->user_id ." and YEAR(created_at) =". $request->tahun. ' ORDER BY id DESC'));
            
            if(!$items)
            {
                $items   = \DB::select(\DB::raw("SELECT * FROM payrollgross_history WHERE user_id=". $data->user_id ." and YEAR(created_at) =". $request->tahun ." ORDER BY id DESC"));
                $dataArray[$k] = $items[0];
            }
            else
            {
        ...
        $view =  view('administrator.request-pay-slipgross.print-pay-slip')->with($params);
        ...
        $objDemo->content = view('administrator.request-pay-slipgross.email-pay-slip'); 
        
        if($data->user->email != "")
        { 
            \Mail::send('administrator.request-pay-slipgross.print-pay-slip', $params,
        ...
        return redirect()->route('administrator.request-pay-slipgross.index')->with('message-success', 'Request Pay Slip berhasil diproses');
    }
}
